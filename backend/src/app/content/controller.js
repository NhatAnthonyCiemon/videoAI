...
const contentController = {
    getContentData: async (req, res) => {
        ...
        try {
            const contentData = await generateScript(topic, style);
            res.json({
                mes: "success",
                status: 200,
                data: contentData,
    ...
    handleVideoController: async (req, res) => {
        try {
            ...
            let audioUrls = [];
            if (is_custom_voice) {
                console.log("🔊 Sử dụng custom voice");
                audioUrls = image_video.map((item) => item.url_mp3);
                console.log(
                    "🔊 Danh sách URL âm thanh từ custom voice:",
                    audioUrls
                );
            }
